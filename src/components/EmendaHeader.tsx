...
export default function EmendaHeader({ emenda }: EmendaHeaderProps) {
  return (
    <div className="bg-white border border-n-2 rounded-lg p-4 md:p-5 mb-[18px] shadow-sm">
      <div className="text-[11px] uppercase tracking-wider text-n-4 font-semibold mb-2">Emenda selecionada</div>
      <div className="flex items-start justify-between gap-3 mb-2.5">
        <div className="text-sm font-medium text-n-6 leading-[1.45] max-w-[85%]">{emenda.objeto}</div>
        <div className="text-[11px] bg-n-1 text-n-4 rounded px-2 py-0.5 border border-n-2 shrink-0">
          {emenda.plano} · {emenda.prazo} meses
        </div>
      </div>

      <div className="text-xs text-n-4 grid sm:grid-cols-3 gap-x-3 gap-y-1 mb-3">
        <span>
      ...
      <div className="flex items-center gap-2">
        <span className="text-[11px] uppercase tracking-wider text-n-4 font-semibold">Valor da emenda</span>
